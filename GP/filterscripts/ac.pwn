...
#define DIALOG_HACKCODES        (3010)
#define DIALOG_REPORTS 			(3011)
#define DIALOG_CMDS 			(3012)
#define DIALOG_CFG 				(3013)
#define DIALOG_VARLIST 			(3014)
#define DIALOG_SETVAR 			(3015)
...
PUBLIC: JB::SetPlayerSkin(playerid, skinid)
{
	if (IsPlayerConnected (playerid))
	{
		    SetSyncTime (playerid, SYNC_TYPE_POS);
			SetPlayerSkin (playerid, skinid);
			JB::PlayerInfo [playerid][JB::pSkin] = skinid;
 			return 1;
    }
	return 0;
}
...
PUBLIC: VerifyNoReload (playerid)
{
	...
	{
		JB::Kick (playerid, "No Reload [Cód 1]");
		return 1;
	}
	return 0;
}
...
PRIVATE: JB::MutePlayer (playerid, time, reason [])//Time is seconds.
{
	if (IsPlayerConnected (playerid) && time)
	{
		...
		JB::SendFormattedMessageToAll (JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} '%s' [%d] ha sido muteado por %d segundos(s) | Razón: {FFFFFF}%s", JB::PlayerInfo [playerid][JB::pName], playerid, time, reason);
		return 1;
	}
	return 0;
}
...
PRIVATE: JB::Kick (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		...
		JB::SendFormattedMessageToAll (JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} %s[%d] ha sido expulsado. [Razón: {FFFFFF}%s]", JB::PlayerInfo [playerid][JB::pName],playerid, reason);
...
PRIVATE: JB::Ban (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		...
		else
		{
			...
			format (string, sizeof (string), "~r~Aviso de Ban: ~n~%s", reason);
			GameTextForPlayer (playerid, string, 60000, 4);
		}

		TogglePlayerControllable (playerid, false);
		SetCameraBehindPlayer (playerid);
		JB::SendFormattedMessageToAll (JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} %s ha sido baneado por el anticheat [Razón: {FFFFFF}%s]", JB::PlayerInfo [playerid][JB::pName], reason);
...
PRIVATE: DisplayKickBanText (playerid, reason [], bool: ban)
{
	if (IsPlayerConnected (playerid))
	{
		...
		{
	        JB::KickBanTitle = TextDrawCreate (320.000000, 100.000000, ban ? ("GP.SEC~n~Ban Info~n~~n~~n~~n~~n~~n~~n~~n~~n~") : ("GP.SEC~n~Kick Info~n~~n~~n~~n~~n~~n~~n~~n~~n~"));
		    ...
		    TextDrawSetString (JB::KickBanTitle, ban ? ("GP.SEC~n~Ban Info~n~~n~~n~~n~~n~~n~~n~~n~~n~") : ("GP.SEC~n~Kick Info~n~~n~~n~~n~~n~~n~~n~~n~~n~"));
		...
		format (string, sizeof (string), "~y~Nick:~n~~w~%s~n~~y~Fecha:~n~~w~%s~n~~y~Hora:~n~~w~%s~n~~y~IP:~n~~w~%s~n~~y~Razon:~n~~w~%s", JB::PlayerInfo [playerid][JB::pName], JB::GetDate (), JB::GetTime (),
		...
		format (string, sizeof (string), "~g~Si ha sido una equivocacion %s, Presiona~b~F8~g~ y envialo a nuestro facebook ~p~%s~g~.", ban ? ("banned") : ("kicked"), JB::Homepage);
...
PUBLIC: CheckText (playerid, text []) //return 1: something forbidden found, return 0: everything OK!
{
	...
	{
		JB::Kick (playerid, "Spam");
		return 1;
	}

	if (JB::Variables [CAPS_LOCK] && CapsLockCheck (text))
	{
		SendClientMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} No tienes permitidas las maýusculas.");
		return 1;
	}

	if (JB::Variables [SINGLEPLAYER_CHEATS] && SingleplayerCheatCheck (text))
	{
		JB::Kick (playerid, "(N00B, ingreso de trucos Singleplayer");
		return 1;
	}

	if (JB::Variables [BAD_WORDS] && BadWordsCheck (text))
	{
		SendClientMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} No estas habilitado para usar esas palabras!");
		return 1;
	}
	return 0;
}
...
PRIVATE: JunkBusterReport (playerid, report [], details [])
{
	if (IsPlayerConnected (playerid))
	{
		...
		format (string, sizeof (string), "[%s] %s [%d, %s] con %s, %s", JB::GetTime (), JB::PlayerInfo [playerid][JB::pName], playerid, JB::PlayerInfo [playerid][JB::pIp], report, details);
		...
		format (string, sizeof (string), "{FE285A}[GP.{FFFFFF}SEC]{FE285A} ¡Atención Admins! %s %s [%d, %s], %s.", report, JB::PlayerInfo [playerid][JB::pName], playerid, JB::PlayerInfo [playerid][JB::pIp], details);
		ForEachPlayer (i)
    ...
    ForEachPlayer (i)
    {
        if (JB::Variables [MAX_TOTAL_WARNINGS])
        {
			...
			if (totalwarnings >= JB::Variables [MAX_TOTAL_WARNINGS])
			{
			    switch (JB::Variables [TOO_MANY_WARNS_ACTION])
			    {
			        case 1:
					{
					//    JB::Kick (i, "Sospecha de Hacks [Cod. 1]");
					    continue;
					}
					
					case 2:
					{
				//	    JB::Ban (i, "Sospecha de Hacks [Cod. 2]");
					    continue;
					}
					
					case 3:
					{
				//	    TempBan (i, 1, "Sospecha de Hacks [Cod. 3]");
					    continue;
					}
			    }
			}
        ...
        if (DOB::GetBit (JB::FullyConnected, i))
        {
				...
				&& (nokeypressed || IsPlayerInRangeOfPoint (i, 3.0, JB::PlayerInfo [i][JB::pAFKPos][0], JB::PlayerInfo [i][JB::pAFKPos][1], JB::PlayerInfo [i][JB::pAFKPos][2])))
	        {
	            ...
	            {
	                format (reason, sizeof (reason), "Tiempo AFK excedido [%d Minutos]", JB::Variables [AFK]);
	                JB::Kick (i, reason);
	            }
	        }
			else
	        ...
	        if (!JB::SyncInfo [i][SYNC_TYPE_POS][JB::sSyncTime] && DOB::GetBit (JB::FullyConnected, i))
	        {
		        if (GetPlayerSurfingVehicleID (i) == INVALID_VEHICLE_ID && GetPlayerSurfingObjectID (i) == INVALID_OBJECT_ID && JB::GetPlayerSpeed (i, true) < 30 && (JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_DRIVER || JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_ONFOOT))
		        {
					...
					if (speed > JB::Variables [AIRBRAKE] && JB::PlayerInfo [i][JB::pLastAirbrakeSpeed] > JB::Variables [AIRBRAKE])
					{
						...
						{
							format (reason, sizeof (reason), "Altura: %.2f, ~%d metros, Modelo: %s", JB::PlayerInfo [i][JB::pOldAirbrakePos][2], speed, JB::GetVehicleName (GetPlayerVehicleID (i)));
                            JB::PlayerInfo [i][JB::pLastAirbrakeSpeed] = 0;
							JunkBusterReport (i, "ABS", reason);
						}
						else
						{
				    ...
				    if (JB::Variables [REPORT_MONEY_HACK] && !JB::IsPlayerAdmin (i))
				    {
                        if (!HasTimePassed (JB::PlayerInfo [i][JB::pLastUpdate], 2000))
                        {
                            ...
                            if (JB::Warnings [i]{MONEY_HACK} >= MAX_CHECKS)
                            {
								...
								{
								    JB::Kick (i, "Money Hack");
								    continue;
								}
								else
								{
	                                format (reason, sizeof (reason), "$%d cambiado a $%d", JB::PlayerInfo [i][JB::pMoney], var);
									JunkBusterReport (i, "MH", reason);
			...
			if (!JB::IsPlayerAdmin (i) && IsPlayerInValidState (i) && !DOB::GetBit (JB::AntiBugKilled, i)) //!JB::PlayerInfo [i][JB::pAntiBugKilled])
			{
			    ...
			    if (JB::Variables [JOYPAD] && !JB::PlayerInfo [i][JB::pFreezeTime])
				{
				    ...
				    if((updown != 128 && updown != 0 && updown != -128) || (leftright != 128 && leftright != 0 && leftright != -128))
				    {
				        if (++JB::Warnings [i]{JOYPAD} >= JB::Variables [JOYPAD])
				        	JB::Kick (i, "Usar Joypad");
						else
						{
						    JB::PlayerInfo [i][JB::pFreezeTime] = 5; // Freeze for 5 seconds.
						    SetSyncTime (i, SYNC_TYPE_VEHICLE);
							TogglePlayerControllable (i, false);
							SendClientMessage (i, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Please stop using a joypad/gamepad. Use your keyboard and mouse.");
						}
				    }
				}
			        ...
			        if (++JB::PlayerInfo [i][JB::pBuyingGuns] >= 2)
			        {
				        if (GetTickCount () - JB::PlayerInfo [i][JB::pLastMoneyChange] > 5000)
				        {
				            if (++JB::Warnings [i]{PAY_FOR_GUNS} >= JB::Variables [PAY_FOR_GUNS])
				            {
				                ...
				                format (reason, sizeof (reason), "No has pagado por el arma: %s", reason);
					            JB::Kick (i, reason);
					            continue;
				            }
				        }
				...
				if (JB::Variables [MIN_FPS])
				{
					...
					if (var < JB::Variables [MIN_FPS])
					{
						...
						{
							format (reason, sizeof (reason), "Mínimo FPS excedido (%d, MIN: %d)", var, JB::Variables [MIN_FPS]);
							JB::Kick (i, reason);
							continue;
						}
						else if (JB::Warnings [i]{MIN_FPS} % (MAX_CHECKS * 10) == 0)
						{
							JB::SendFormattedMessage (i, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Por favor, aumenta tus Framerates (FPS) o serás expulsado! (Min %d, FPS: %d)", JB::Variables [MIN_FPS], var);
						}
					}
				}
						...
						if (JB::PlayerInfo [i][JB::pCurrentState] != PLAYER_STATE_PASSENGER && !surfing && !IsPlayerInRangeOfPoint (i, 350.0, JB::PlayerInfo [i][JB::pCurrentPos][0], JB::PlayerInfo [i][JB::pCurrentPos][1], JB::PlayerInfo [i][JB::pCurrentPos][2]))
						{
							if (!IsPlayerInRangeOfPoint (i, 5.0, JB::PlayerInfo [i][JB::pSetPos][0], JB::PlayerInfo [i][JB::pSetPos][1], JB::PlayerInfo [i][JB::pSetPos][2]))
							{
								...
								if (JB::Variables [TELEPORT_HACK])
								{
									...
									{
										format (reason, sizeof (reason), "Teleport Hack [Cod. 1]: %s", reason);
										JB::Ban (i, reason);
										continue;
									}
									...
									{
									    format (reason, sizeof (reason), "Teleport Hack [Cód. 3]: %s", reason);
										JB::Ban (i, reason);
										continue;
									}
									else if (JB::Warnings [i]{TELEPORT_HACK} >= MAX_CHECKS)
									{
										format (reason, sizeof (reason), "Teleport Hack [Cod. 2]: %s", reason);
										JB::Kick (i, reason);
										continue;
									}
									else
										JunkBusterReport (i, "TH", reason);
								}
								else
									JunkBusterReport (i, "TH", reason);
							}
					...
					if (!JB::AirbrakeDetection && JB::Variables [AIRBRAKE] && !surfing && (JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_ONFOOT || (JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_DRIVER && JB::GetPlayerSpeed (i, true) < 30)))
					{
						...
						if (var >= JB::Variables [AIRBRAKE])
						{
							...
							if (JB::PlayerInfo [i][JB::pAirbraking] >= (MAX_CHECKS - 1))
							{
								...
								if (JB::Warnings [i]{AIRBRAKE} < MAX_CHECKS)
								{
									format (reason, sizeof (reason), "Altura: %.2f, ~%d m/s, Modelo: %s", z, var, JB::GetVehicleName (vehicleid));
									JunkBusterReport (i, "ABS", reason);
								}
								else
								{
						    ...
						    if (JB::Variables [SPEEDHACK_DETECTION] == 0)
						    {
								...
								if (!JB::PlayerInfo [i][JB::pFreezeTime])
								{
									if (var > JB::Variables [MAX_SPEED])
									{
										...
										{
											format (reason, sizeof (reason), "%d KM/H - Modelo: %s (MAX: %d)", var, JB::GetVehicleName (vehicleid), ivar [0]);
											JunkBusterReport (i, "SH [Cód 1]", reason);
										}
										else
										{
											JB::Ban (i, "Speed Hack [Cód. 1]");
											continue;
										}
									}
									else if (JB::Variables [SPEEDHACK_ADVANCED] && var > (ivar [0] + JB::Variables [SPEEDHACK_ADVANCED]))
									{
										...
										{
											format (reason, sizeof (reason), "%d KM/H Modelo: %s (MAX %d)", var, JB::GetVehicleName (vehicleid), ivar [0]);
											JunkBusterReport (i, "SH [Cod. 2]", reason);
										}
										else
										{
											JB::Kick (i, "Speed Hack [Cód 2]");
											continue;
										}
									}
								}
							    ...
							    if (75.0 < abs (rz) < 105.0)
							    {
									...
									if (floatround (rz * 160.0) < JB::Variables [WALLRIDE] && var > 10)
									{
									    ...
									    if (JB::PlayerInfo [i][JB::pWallriding] == MAX_CHECKS)
									    {
										    ...
										    {
										        JB::Kick (i, "Wallride Hack");
										        continue;
										    }
										    else
										    {
										        format (reason, sizeof (reason), "con %s", JB::GetVehicleName (vehicleid));
												JunkBusterReport (i, "WR", reason);
										    }
									    }
									}
							    }
				...
				if (IsWeaponForbiddenForPlayer (i, var))
				{
					...
					format (reason, sizeof (reason), "Weapons Hack (%s)", reason);
					JB::Ban (i, reason);
					continue;
				}
				...
				if (JB::Variables [WEAPON_HACK] && !JB::SyncInfo [i][SYNC_TYPE_WEAPONS][JB::sSyncTime])
				{
					if (var >= 16 && var <= 39)//Grenades and guns
					{
						...
						{
							GetWeaponName (var, reason, sizeof (reason));
							format (reason, sizeof (reason), "Weapons Hack [Cód 1] [%s]", reason);
							JB::Kick (i, reason);
							continue;
						}
					}
				...
				if (JB::PlayerInfo [i][JB::pPingCheckProgress] == MAX_PING_CHECKS)
				{
					if (JB::Variables [MAX_PING])
					{
						...
						{
							format (reason, sizeof (reason), "Ping MAX. excedido. [%d - %d]", var, JB::Variables [MAX_PING]);
							JB::Kick (i, reason);
							continue;
						}
					}
...
PRIVATE: SplitIp (ipstring [])
{
	...
	PUBLIC: UnbanIp (ipstring [])
	{
		...
		for (new i = 0; i < IpBanCount; ++i)
		{
			if (IsSameIp (IpBans [i], ip))
			{
				...
				JB::LogEx ("IP %s ha sido desbaneada.", ipstring);
				SaveIpBans ();
				return 1;
			}
		}
	...
	PUBLIC: AddPlayerToBlacklist (playerid)
	{
		if (AddNameToBlacklist (JB::PlayerInfo [playerid][JB::pName]))
			return JB::Ban (playerid, "Black List");
		return 0;
	}
		...
		for (new i = 0; i < TempBanCount; ++i)
		{
			if (!strcmp (TempBanInfo [i][tbName], JB::PlayerInfo [playerid][JB::pName], false))
			{
				if (TempBanInfo [i][tbTime] > time)
				{
					...
					JB::SendFormattedMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Has sido baneado temporalmente por %d day(s), %d hour(s), %d minute(s) and %d second(s)!", days, hours, minutes, seconds);
					JB::LogEx ("%s (%s) ha sido baneado por Ban Evade.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp]);
...
PRIVATE: ShowPlayerHackCodeDialog (playerid)
{
	...
	strcat (string, "{0CB7EB}No Reload:{FFFFFF}\n\tCód 1: Probablemente, use 2-Shoot.\n\tCód 2: Dispara demasiado rápido con la Sawn, posible Hack.\n\n");
	strcat (string, "{0CB7EB}TH:{FFFFFF}\n\tCód 1: Se ha transportado a un lugar de manera sospechosa\n\tCód 2: Posición no definida, mezcla de coordenadas, ¿Fué hacia otro jugador?\n\tCód 3: Fue hacia un Pickup.\n\n");
	strcat (string, "{0CB7EB}SH:{FFFFFF}\n\tCód 1: Excedió la velocidad de la variable GP.SEC (Nitro o pendiente).\n\tCód 2: Excedió la velocidad del Handle del vehículo.\n\n");
	strcat (string, "{0CB7EB}HH/AH:{FFFFFF}\n\tCód 1: Es mayor a 100.\n\tCód 2: Es mayor a lo que GP.SEC esperaba.");
	ShowPlayerDialog (playerid, DIALOG_HACKCODES, DIALOG_STYLE_MSGBOX, "Codigos de GP.SEC", string, "Cerrar", "");
}
...
CALLBACK: OnPlayerGodMode (playerid)
{
	if (JB::Variables [ACTIVE_GMC] && !JB::IsPlayerAdmin (playerid))
	{
		...
		if (JB::Warnings [playerid]{ACTIVE_GMC} >= JB::Variables [ACTIVE_GMC])
		{
			if (JB::Variables [GMC_BAN])
				JB::Ban (playerid, "God Mode");
			else
				JB::Kick (playerid, "God Mode");
			return 1;
		}
		else
	...
	JunkBusterReport (playerid, "GM", "no details");
	return 1;
}
	    ...
	    new Float: x, Float: y, Float: z;
 		...
 		if (GetVehicleDistanceFromPoint (vehicleid, x, y, z) < 5.0)
 		{
 		    if (++JB::Warnings [playerid]{SPAWN_VEHICLES} >= JB::Variables [SPAWN_VEHICLES])
 		    {
 		        ...
 		        JunkBusterReport (playerid, "SV", JB::GetVehicleName (vehicleid));
 		}
	}
...
CALLBACK: OnPlayerSpawn (playerid)
{
	...
	if (DOB::GetBit (JB::AntiBugKilled, playerid))
	{
		...
		JB::SetPlayerPos (playerid, GetPVarFloat (playerid, "JB_ABK_PosX"), GetPVarFloat (playerid, "JB_ABK_PosY"), GetPVarFloat (playerid, "JB_ABK_PosZ"));
	    ...
	    if (JB::PlayerInfo [playerid][JB::pLastVehicle] != 0 && vehicleid != 0 && !JB::IsPlayerAdmin (playerid))
	    {
	        if (JB::PlayerInfo [playerid][JB::pLastVehicle] != vehicleid)
	        {
			    if (!JB::SyncInfo [playerid][SYNC_TYPE_POS][JB::sSyncTime])
			    {
					if (JB::Variables [VEHICLE_TELEPORT])
					{
			            ...
			            JunkBusterReport (playerid, "VT", JB::GetVehicleName (vehicleid));
			    }
		    }
		    else if (JB::Variables [VEHICLE_REPAIR] && !JB::SyncInfo [playerid][SYNC_TYPE_VEHICLE][JB::sSyncTime])
		    {
		        if (fvar > JB::PlayerInfo [playerid][JB::pVehicleHealth])
		        {
		            ...
		            {
		                JB::Kick (playerid, "Repair Vehicle");
		                return 0;
		            }
		            else
		                JunkBusterReport (playerid, "Repair Vehicle", JB::GetVehicleName (vehicleid));
		        }
		    }
	    }
	...
	if (!JB::IsPlayerAdmin (playerid))
	{
		...
		if (JB::Variables [SPEEDHACK_DETECTION] != 0 && JB::PlayerInfo [playerid][JB::pCurrentState] == PLAYER_STATE_DRIVER && !JB::PlayerInfo [playerid][JB::pFreezeTime])
		{
			...
			if (speed > JB::Variables [MAX_SPEED])
			{
				...
				SetSyncTime (playerid, SYNC_TYPE_VEHICLE);
				JB::TogglePlayerControllable (playerid, false);
				if (JB::Warnings [playerid]{MAX_SPEED} < MAX_CHECKS)
				{
					format (reason, sizeof (reason), "%d KM/H - Modelo: %s (Max %d)", speed, JB::GetVehicleName (vehicleid), maxspeed);
					JunkBusterReport (playerid, "SH [Code 1]", reason);
				}
				else
				{
			...
			else if (JB::Variables [SPEEDHACK_ADVANCED] && speed > (maxspeed + JB::Variables [SPEEDHACK_ADVANCED]))
			{
				...
				{
					format (reason, sizeof (reason), "%d KM/H - Modelo: %s (Max %d)", speed, JB::GetVehicleName (vehicleid), maxspeed);
					JunkBusterReport (playerid, "SH [Code 2]", reason);
				}
				else
				{
					JB::Kick (playerid, "Speed Hack [Code 2]");
					return 0;
				}
			}
		}
				...
				else
				{
					...
					if (JB::PlayerInfo [playerid][JB::pAmmoUsed][ivar] > (AmmoAmount [ivar] * 2) && ammoused > 0)//Player must have switch weapons fast or doesn't reload at all
					{
						...
						if ((JB::Variables [NO_RELOAD] == 1 && (JB::Warnings [playerid]{NO_RELOAD} % 10) == 0))
						{
							...
							JunkBusterReport (playerid, "NR", reason);
						}
		    ...
		    if (JB::PlayerInfo [killerid][JB::pCurrentState] == PLAYER_STATE_DRIVER)
		    {
				if (JB::Variables [DRIVE_BY] >= 2 && (reason == WEAPON_UZI || reason == WEAPON_MP5 || reason == WEAPON_TEC9 || reason == 50)) // 50 = helicopter blades.
				{
				    if (++JB::Warnings [killerid]{DRIVE_BY} >= MAX_CHECKS)
				        JB::Kick (killerid, "Excesivo Drive-By");
					else
					{
						JB::SetPlayerHealth (killerid, 0.0);
						SendClientMessage (killerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Has sido expulsado por hacer Drive-By");
					}
				}
			 	...
			 	if (JB::Variables [ARMED_VEHICLES])
			 	{
					...
					if (IsArmedVehicle (vehicleid))
					{
					    if (++JB::Warnings [killerid]{ARMED_VEHICLES} >= JB::Variables [ARMED_VEHICLES])
						{
						    ...
						    format (string, sizeof (string), "Asesinatos excesivos con %s", JB::GetVehicleName (vehicleid));
						    JB::Kick (killerid, string);
						}
						else
							JB::SendFormattedMessage (killerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Asesinatos con la %s no permitidos, para! (Advertencias %d/%d)", JB::GetVehicleName (vehicleid), JB::Warnings [killerid]{ARMED_VEHICLES}, JB::Variables [ARMED_VEHICLES]);
					}
			 	}
			}
			...
			if (JB::Variables [SPAWNKILL])
			{
				if (JB::PlayerInfo [playerid][JB::pSpawnKillProtected])
				{
					if (++JB::Warnings [killerid]{SPAWNKILL} >= JB::Variables [SPAWNKILL])
						...
						JB::SendFormattedMessage (killerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} SpawnKill no permitido (Advertencias %d/%d)", JB::Warnings [killerid]{SPAWNKILL}, JB::Variables [SPAWNKILL]);
				}
		 	}
		}
	...
	if (!IsPlayerNPC (playerid))
	{
	        ...
	        if (JB::Variables [BLACKLIST])
	        {
		        ...
		        if (banned)
		        {
		            ...
		            {
		                SendClientMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Has sido baneado del servidor!");
					...
					else if (t < unbantime)
					{
					    ...
					    JB::SendFormattedMessageToAll (JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Has sido baneado temporalmente por %d day(s), %d hour(s), %d minute(s) and %d second(s)!", days, hours, minutes, seconds);
...
CALLBACK: OnPlayerText (playerid, text [])
{
	...
	{
		JB::SendFormattedMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} No puedes escribir por %d segundo(s) porque estás silenciado!", JB::PlayerInfo [playerid][JB::pMuted]);
		return 0;
	}
...
public OnPlayerCommandReceived(playerid, cmdtext[])
{
	...
	if (JB::Variables [COMMAND_SPAM] && (!JB::IsPlayerAdmin (playerid) || !JB::Variables [ADMIN_IMMUNITY]))
	{
		...
		{
			JB::Ban (playerid, "Comando SPAM extremo");
			return 0;
		}
		else if (JB::PlayerInfo [playerid][JB::pCommands] > 5)
		{
   JB::Kick (playerid, "Comando SPAM");
   return 0;
		}
	}
	return 1;
...
//==============================================================================
	...
	COMMAND:jbtest (playerid, params [])
	{
		...
		t = GetTickCount () - t;
		JB::SendFormattedMessage (playerid, JB_RED, "time: %d ms", t);
		return 1;
	}
...
//	#pragma unused params
	...
	ShowPlayerDialog (playerid, DIALOG_CMDS, DIALOG_STYLE_LIST, "Commands", string, "Perform", "Close");
	return 1;
}
...
COMMAND:jbchrome (playerid, params [])
{
	...
	if (JunkBusterChrome (id, JB::PlayerInfo [playerid][JB::pName]))
		JB::SendFormattedMessage (playerid, JB_GREEN, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Chrome has successfully collected data of player '%s'!", JB::PlayerInfo  [id][JB::pName]);
	else
	    JB::SendFormattedMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Chrome could not colect data of player '%s'!", params);
	return 1;
}
...
COMMAND:jbsethomepage (playerid, params [])
{
    ...
    DOF2::SetString (CONFIG_FILE, "Homepage", JB::Homepage, "Misc");
    JB::SendFormattedMessage (playerid, JB_GREEN, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Homepage has been set to '%s'!", params);
	return 1;
}
...
COMMAND:blackadd (playerid, params [])
{
	...
	if (!IsPlayerConnected (id))
	{
		...
		#endif
				JB::SendFormattedMessageToAll (JB_GREEN, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} '%s' ha sido publicado en la lista negra del servidor!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Could not add player '%s' to blacklist!", params);
	}
	else
	{
		if (id != playerid)
		{
		        ...
		        {
		            JB::SendFormattedMessage (playerid, JB_GREEN, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Player '%s' has successfully been added to blacklist!", JB::PlayerInfo [id][JB::pName]);
		            JB::Kick (id, "Black List");
		        }
		    #else
				if (AddPlayerToBlacklist (id))
					...
					JB::SendFormattedMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Could not add player '%s' to blacklist!", JB::PlayerInfo [id][JB::pName]);
		}
	}
	return 1;
}
...
COMMAND:blackdel (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Player '%s' has successfully been removed from blacklist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Could not remove player '%s' from blacklist!", params);
			return 1;
	}
...
COMMAND:whiteadd (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Player '%s' has successfully been added to whitelist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Could not add player '%s' to whitelist!", params);
			return 1;
	}
...
COMMAND:whitedel (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Player '%s' has successfully been removed from whitelist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Could not remove player '%s' from whitelist!", params);
			return 1;
	}
...
COMMAND:jbsetvar (playerid, params [])
{
	...
	{
	    JB::SendFormattedMessage (playerid, JB_GREEN, "{FE285A}[GP.{FFFFFF}SEC]{FE285A}  '%s' has successfully been set to %d.", var, value);
...
COMMAND:tban (playerid, params [])
{
	...
	if (IsPlayerConnected (id) && id != playerid && days > 0 && !isnull (reason))
	{
	        ...
	        format (reason, sizeof (reason), "%s [%d dia(s) prohibido]", reason, days);
	        JB::Kick (id, reason);
	    #else
...
COMMAND:tunban (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Temporary ban of player '%s' has successfully been deleted.", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Could not delete temporary ban of player '%s'!", params);
			return 1;
	}
...
COMMAND:banip (playerid, params [])
{
	if (!IsPlayerAdmin (playerid))
	    ...
	    SendClientMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Sorry, this command is currently not available when using JunkBuster database.");
	    return 1;
	#else
		if (!isnull (params))
		{
			...
			{
				if (BanIp (params))
					JB::SendFormattedMessage (playerid, JB_GREEN, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} IP %s has successfully been banned!", params);
				else
					JB::SendFormattedMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Could not ban IP %s!", params);
				return 1;
			}
		}
...
COMMAND:unbanip (playerid, params [])
{
	if (!IsPlayerAdmin (playerid))
	    ...
	    if (JBDB::UnbanIp (params))
			JB::SendFormattedMessage (playerid, JB_GREEN, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} IP %s has successfully been unbanned!", params);
		else
			JB::SendFormattedMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Could not unban IP %s!", params);
		return 1;
	#else
		if (!isnull (params))
		{
		...
		if (JB::PlayerInfo [playerid][JB::pVehicleEntered] != vehicleid && !JB::IsPlayerAdmin (playerid) && !IsPlayerNPC (playerid))
		{
			...
			if (driver != INVALID_PLAYER_ID)
			{
				...
				if (JB::Variables [CAR_JACK_HACK] && JB::Warnings [playerid]{CAR_JACK_HACK} >= MAX_CHECKS)
					...
					JunkBusterReport (playerid, "CJH", JB::PlayerInfo [driver][JB::pName]);
			}
				...
				case 24, 25, 27, 29, 30, 31, 33, 34: // Deagle, Shotung, SPAS12, MP5, AK47, M4, Rifle, Sniper Rifle
				{
					...
					else if (((oldkeys & KEY_FIRE) || (oldkeys == KEY_FIRE)) && ((newkeys & KEY_CROUCH) || (newkeys == KEY_CROUCH)) && (GetTickCount () - JB::PlayerInfo [playerid][JB::pFired]) < 750)
					{
						if (++JB::Warnings [playerid]{CBUG} & 1 == 0)
							SendClientMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Please stop performing the C-Bug or you will get kicked/banned.");
	...
	if (1 <= JB::Variables [DRIVE_BY] <= 2 && PRESSED (KEY_FIRE) && (HOLDING (KEY_LOOK_RIGHT) || HOLDING (KEY_LOOK_LEFT)))
	{
		if (!IsPlayerInPlane (playerid) && GetVehicleModel (GetPlayerVehicleID (playerid)) != 432)//Ignore planes and rhinos.
		{
			...
			if (weaponid != 0 && ammo != 0)//Check if player has got an SMG. Don't punish innoncent players.
			{
				...
				SendClientMessage (playerid, JB_RED, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Deja de hacer Drive-By!");
				TogglePlayerControllable (playerid, false);
			...
			case DIALOG_CFG:
			{
				if (response)
				{
					switch (listitem)
					{
						...
						{
							ConfigJunkBuster ();
							SendClientMessage (playerid, JB_GREEN, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Configuration has been loaded from file.");
							ShowPlayerConfigDialog (playerid);
						}
						...
						{
							SaveJunkBusterVars ();
							SendClientMessage (playerid, JB_GREEN, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Configuration has been saved to file.");
							ShowPlayerConfigDialog (playerid);
						}
						...
						{
							JB::Variables = JB::DefaultVariables;
							SendClientMessage (playerid, JB_GREEN, "{FE285A}[GP.{FFFFFF}SEC]{FE285A} Default configuration has been loaded.");
	    					JB::Log ("s have been reset to default values.");
							ShowPlayerConfigDialog (playerid);
						}
			...
			case DIALOG_SETVAR .. (DIALOG_SETVAR + MAX_JB_VARIABLES - 1):
			{
				if (response)
				{
					...
					else
					{
						...
						JB::SendFormattedMessage (playerid, JB_GREEN, "{FE285A}[GP.{FFFFFF}SEC]{FE285A}  '%s' has successfully been set to %d.", JB::VariableNames [var], JB::Variables [var]);
...
CALLBACK: OnRconCommand (cmd [])
{
	...
	{
	    JB::Variables = JB::DefaultVariables;
	    JB::Log (" have been reset to default values by an RCON admin.");
	    return 1;
	}
		...
		if (JB::Variables [CHECKPOINT_TELEPORT])
		{
			if (!JB::GetPlayerSpeed (playerid, false))
			{
				...
				if (JB::Warnings [playerid]{CHECKPOINT_TELEPORT} >= MAX_CHECKS)
					JB::Ban (playerid, "Checkpoints Teleport ");
				else
				{
					new string [128];
						
					format (string, sizeof (string), "Checkpoint TH (Advertencias %d)", JB::Warnings [playerid]{CHECKPOINT_TELEPORT});
					JunkBusterReport (playerid, "TH", string);
				}
			}
		}
	}
